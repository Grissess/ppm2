...
do
	...
	hook.Add 'PreRender', 'PPM2.HornEffects', ->
		for pl in *player.GetAll()
			...
			if data\GetRace() == PPM2.RACE_UNICORN
				...
				if data.flightController.lastState
					if not hornGlowStatus[pl]
						hornGlowStatus[pl] = {}
					if not hornGlowStatus[pl].flying
						hornGlowStatus[pl].flying = {
							:data
							isEnabled: true
							attach: pl\LookupAttachment 'eyes'
							nextSmokeParticle: 0
