...
PPM2.GetMagicAuraColor = =>
	if @GetSeparateMagicColor()
		...
		if not @GetSeparateEyes()
			return DLib.LerpColor(0.5, @GetEyeIrisTop(), @GetEyeIrisBottom())
		else
			left = DLib.LerpColor(0.5, @GetEyeIrisTopLeft(), @GetEyeIrisBottomLeft())
			right = DLib.LerpColor(0.5, @GetEyeIrisTopRight(), @GetEyeIrisBottomRight())
			return DLib.LerpColor(0.5, left, right)
